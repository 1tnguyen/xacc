...
inline Operator ConvertOperatorFromString(const std::string &str) {
  ...
  if (strToOpMap.empty()) {
    for (int enumIter = static_cast<int>(Operator::X);
         enumIter != static_cast<int>(Operator::NA); ++enumIter) {
...
public:
  ...
  virtual void collect(std::string &io_staticHstr,
                       std::vector<std::string> &io_ctrlHstr,
                       std::vector<std::string> &io_channelNames) = 0;
};

class HamiltonianSumTerm : public HamiltonianTerm {
public:
  static std::unique_ptr<HamiltonianTerm>
  fromString(const std::string &in_string, const VarsMap &in_vars);
  ...
  virtual void collect(std::string &io_staticHstr,
                       std::vector<std::string> &io_ctrlHstr,
                       std::vector<std::string> &io_channelNames) override;
...
public:
  ...
  HamiltonianTimeIndependentTerm(const std::complex<double> &in_coeff,
                                 const std::vector<QubitOp> &in_ops)
      : m_coefficient(in_coeff), m_operators(in_ops) {}
...
public:
  ...
  HamiltonianTimeDependentTerm(const std::string &in_channelName,
                               double in_coeff, const QubitOp &in_op)
      : m_channelName(in_channelName), m_coefficient(in_coeff),
        m_operators({in_op}) {}
